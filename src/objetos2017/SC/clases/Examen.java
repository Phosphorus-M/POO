...
public class Examen {
	...
	public Fecha IngresarFecha(Scanner fechas) {
		...
		{
			System.out.println("Ingresar el dia del examen");
			Integer dia = fechas.nextInt(); //Se lee el dia
			...
			Integer ano = fechas.nextInt(); //Se lee el ano
			this.Fecha = new Fecha(dia, mes, ano);//Asignara los valores a Fecha
	...
	public static void main(String[] args) {
		...
		while(!nombre.equals("Salir")){
			...
			if(!nombre.equals("Salir")) {
				...
				Examen examen1 = new Examen(nombre, nota, fechas, aula);//Se crea un objeto con los valores nombre y nota, pero ademas se envia el scanner de fecha
		...
		for(Integer i=0; i < examen.size(); i++) {//Imprimira la cantidad de examenes realizados
			System.out.println("Materia: " + examen.get(i).materia + " Fecha: " + examen.get(i).Fecha.MostrarDia() + "/" + examen.get(i).Fecha.MostrarMes() + "/" +examen.get(i).Fecha.MostrarAno() + " Nota:" + examen.get(i).nota+ " " + examen.get(i).Fecha.Mostrafecha() + " Aula:" + examen.get(i).aula);
		}
